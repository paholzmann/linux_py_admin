from src.users import SingleUsers
...
class UtilitiesCLI:
    ...
    def setup_utilities_cli(self):
        ...
        for definition in self.definitions:
            parser = self.subparser.add_parser(definition["parser"], help=definition["help"])
            actions_parser = parser.add_subparsers(dest="action", required=True)
            for action in definition["actions"]:
                action_parser = actions_parser.add_parser(action["name"], help=action["help"])
